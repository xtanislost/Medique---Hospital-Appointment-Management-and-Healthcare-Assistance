...
const DoctorReviews = ({ docId }) => {
  // Changed prop name to docId to match Appointment
    ...
    const fetchReviews = async () => {
      ...
      try {
        const response = await fetch(
          `${backendUrl}/api/review/doctors/${docId}/reviews`
        ); // Use docId prop
        if (!response.ok) {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      if (!fetchReviewsResponse.ok) {
        throw new Error(
          `Failed to refetch reviews after submission: ${fetchReviewsResponse.status}`
        );
      }
    ...
    <div style={{ width: "100%", fontFamily: "sans-serif" }}>
      ...
      >
        ...
        {reviewForm.submissionError && (
          <p
            style={{ marginTop: "1rem", fontSize: "0.8rem", color: "#e74c3c" }}
          >
            {reviewForm.submissionError}
          </p>
        )}
        {reviewForm.submissionSuccess && (
          <p
            style={{ marginTop: "1rem", fontSize: "0.8rem", color: "#28a745" }}
          >
